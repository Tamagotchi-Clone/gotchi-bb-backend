...
module.exports = class PetScore {
  ...
  static async getAllScores() {
    const { rows } = await pool.query(
      `
      SELECT users.username AS user, COALESCE(user_pets.name, 'noname') AS pet, COALESCE(pet_scores.hunger, 0) AS fed, COALESCE(pet_scores.play, 0) AS played, COALESCE(pet_scores.cleanliness, 0) AS cleaned, SUM(pet_scores.play + pet_scores.cleanliness + pet_scores.hunger) AS happiness
      FROM users
      LEFT JOIN user_pets
