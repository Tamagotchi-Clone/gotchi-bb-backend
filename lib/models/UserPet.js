...
module.exports = class UserPet {
  ...
  static async insert({ userId, petId, name, hunger, play, cleanliness }) {
    const { rows } = await pool.query(
      ' INSERT INTO userPets (user_id, pet_id, name, hunger, play, cleanliness) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;',
      [userId, petId, name, hunger, play, cleanliness]
    );
  ...
  static async deleteUserPetById(id) {
    const { rows } = await pool.query(
      'DELETE FROM userPets WHERE id=$1 RETURNING *',
      [id]
    );
    return new UserPet(rows[0]);
  }
