...
module.exports = Router()
  .post('/', authenticate, async (req, res, next) => {
    try {
      const pet = await UserPet.insert({ ...req.body, userId: req.user.id });
      res.send(pet);
    } catch (error) {
      next(error);
    }
  ...
  .patch('/:id/hunger', authenticate, async (req, res, next) => {
    const today = new Date();
    try {
      const update = await UserPet.updateUserPetById(
        req.params.id,
        req.user.id,
        {
          hunger: today,
  ...
  .get('/users/:id', async (req, res, next) => {
    try {
      const pet = await UserPet.getUserPetByUser(req.params.id);
      res.send(pet);
    } catch (error) {
      next(error);
    }
  ...
  .patch('/:id', authenticate, async (req, res, next) => {
    try {
      const pet = await UserPet.updateUserPetById(
        req.params.id,
        req.user.id,
        req.body
      );
  ...
  .delete('/:id', authenticate, async (req, res, next) => {
    try {
      const pet = await UserPet.deleteUserPetById(req.params.id, req.user.id);
      res.send(pet);
    } catch (error) {
      next(error);
    }
